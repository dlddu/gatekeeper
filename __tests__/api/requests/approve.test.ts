...
describe('PATCH /api/requests/:id/approve', () => {
  ...
  describe('이미 처리된 상태 (409 Conflict)', () => {
    ...
    it('이미 REJECTED 상태를 승인 시도하면 409를 반환해야 한다', async () => {
      mockUserFindUnique.mockResolvedValue(mockUser);
  ...
  describe('정상 승인 (200 OK)', () => {
    it('PENDING 요청을 승인하면 200을 반환해야 한다', async () => {
      ...
      mockRequestUpdate.mockResolvedValue(makeMockRequest({ status: 'APPROVED', processedAt: new Date(), processedById: 'user-admin' }));
      const request = makeRequest('clq1234567890', 'uid-admin-001');
      const params = makeParams('clq1234567890');
      const response = await PATCH(request, params);
    ...
    it('update를 APPROVED 상태로 호출해야 한다', async () => {
      mockUserFindUnique.mockResolvedValue(mockUser);
      ...
      expect(mockRequestUpdate).toHaveBeenCalledWith(expect.objectContaining({
        ...
        data: expect.objectContaining({ status: 'APPROVED', processedAt: expect.any(Date) }),
      }));
    });
  });
});
